...
### 3. **다형성(Polymorphism)**
- **정의**: 
  - 같은 타입의 참조 변수가 실제 어떤 객체를 가리키느냐에 따라 다른 동작
  - 동적 바인딩(실행 시점에 메서드 결정)이 핵심.
- **종류**:  
  - overloading : 컴파일 타임 다형성
  - overriding : 런타임 다형성
...
### 4. **추상화(Abstraction)**
- **정의**: 
  - 핵심 동작만 정의하고 세부 구현은 숨김.
  - 예: List 인터페이스를 쓰면 ArrayList, LinkedList 내부 구현은 몰라도 됨.
- **방법**: 인터페이스, 추상 클래스
- **목적**: 구현 교체 용이, 역할 중심 설계
- interface variable  
  - 무조건 상수 : public static final 자동(강제)
  - 이 말은 인스턴스 필드를 못가진다는 얘기!!! -> **이래서 (중간??)구현체가 필요한거야. default method 가 있다해도**
- interface method(java 8 이상)
  - public abstract 자동
  - default 가능 : 
    - 자동으로 public 붙음  
    - 여기서 default 는 접근제어자 아님. 
    - 인터페이스에 기본 구현을 제공하여, 구현 클래스에서 선택적으로 재정의 가능하게 하기 위함
  - static 가능 : 자동으로 public 붙음. 인터페이스 이름으로만 호출 가능(구현체 인스턴스로 호출 불가)
  - private 가능(java 9 이상) : 인터페이스 내부에서만 사용가능 (default , static 보조용)
    - default/static 메서드 간 코드 중복 제거 + 캡슐화
```java
interface Payment {
    void pay(int amount);
}
